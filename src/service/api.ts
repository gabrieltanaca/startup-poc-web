...
function makeUrl(path = '') {
    ...
    const ambient = process.env.NEXT_PUBLIC_APP_ENV || '';
    ...
    const useProxy = ambient === 'production' && !apiURL.includes('https://');

    if (useProxy) {
        return ['/api/proxy', ...path.trim().split('/').filter(Boolean)].join('/');
    }
